...
int main(int argc, char *argv[])
{
	...
	void11.ctrl_sock = -1;
	void11.conf->ssid.ssid_len = sizeof(void11.conf->ssid);
	memset(void11.conf->ssid, 0, void11.conf->ssid.ssid_len - 1);
	...
	for(;;) {
		...
		switch (c) {
		case 'S':
			void11.conf->ssid.ssid_len = strlen(optarg);
			if(void11.conf->ssid.ssid_len >
			   (sizeof(void11.conf->ssid.ssid) - 1) ||
			   void11.conf->ssid.ssid_len < 1) {
				DPRINT("Invalid SSID '%s'\n", optarg);
				usage();
			}
			match_ssid = 1;
			strncpy(void11.conf->ssid.ssid,
				optarg,
				void11.conf->ssid.ssid_len);
			break;
		case 's':
			...
			staptr = (u8*)&station;//station == stapr == MAC-of-Victim
			break;
		case 't':
			type = atoi(optarg);
			if(type < 0 || type > 3) {
